...
export async function run(args: string[]) {
  ...
  const [action, target, name, codeBranch] = args;
  switch (action) {
    case 'init':
      if (!target || !name) throw new Error('Usage: alphabook init /absolute/repo "Project name" [code-branch]');
    ...
    case 'write': {
      if (!target) throw new Error('Usage: alphabook write request.json (or - for stdin). See agent primer for request fields.');
    ...
    default: throw new Error('Usage: alphabook init|register|projects|show|validate|write. MCP and the web server are optional.');
  }
}
if (import.meta.main) {
