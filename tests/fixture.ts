...
import { commandHead, digest, git, Registry } from '../packages/core/index';
import { importPlanning, writePlanning } from '../packages/core/write';

export function fixture() {
  const dir = mkdtempSync(join(tmpdir(), 'command-test-')), root = join(dir, 'project');
  mkdirSync(root);
  ...
  const files = new Map([
    ['.command/project.yaml', 'format: repo-project\nformat_version: "0.2"\nid: b556899e-eb59-4e98-89c4-157e62fddf1e\nname: Test project\nplanning_branch: command\ncode_branch: main\n'],
    ['.command/tasks/T-001.md', task('T-001', 'planned')],
    ['.command/tasks/T-002.md', task('T-002', 'planned', 'depends_on: [T-001]\n')],
    ['.command/decisions/D-001.md', '---\nkind: decision\nid: D-001\ntitle: Keep files portable\nstatus: accepted\n---\nDecision body.\n'],
    ['.command/docs/guide.md', '# Guide\n\nOrdinary Git files work.\n'],
    ['.command/artifacts/result.txt', 'Passed.'],
  ]);
  ...
  const write = (path: string, content: string | null, subject = 'Update planning') => {
    const head = commandHead(project.commonDir);
  ...
  const rawPlan = (changes: Map<string, string>, subject = 'External Git planning edit') => {
    const temp = mkdtempSync(join(dir, 'index-')), before = commandHead(project.commonDir);
    ...
    try {
      ...
      run(['update-ref', 'refs/heads/command', next, before]); return next;
