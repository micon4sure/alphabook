...
import { commandHead, git, snapshot, worktrees } from '../packages/core/index';
...
function run(root: string, action: string, path: string, head: string, input?: string) {
  return spawnSync('bash', ['-c', script!], {
    ...
    env: {
      ...process.env, COMMAND_ACTION: action, COMMAND_RECORD: path,
      COMMAND_EXPECTED_HEAD: head, COMMAND_INPUT: input || '',
      COMMAND_TOOL_ROOT: resolve('.'), COMMAND_MESSAGE: 'Test the published primer\n\nTask: T-001',
    },
  });
}
function succeeded(result: ReturnType<typeof run>) {
  expect({ status: result.status, stderr: result.stderr }).toEqual({ status: 0, stderr: '' });
  expect(result.stdout).toContain('Published command commit:');
}
...
test('primer Git CRUD creates, reads, updates and deletes without touching code', () => {
  ...
  const path = '.command/docs/from-primer.md';
  writeFileSync(input, '# Created by the primer\n');
  succeeded(run(f.root, 'upsert', path, commandHead(f.project.commonDir), input));
  expect(git(f.root, ['show', `command:${path}`])).toBe('# Created by the primer\n');
  writeFileSync(input, '# Updated\n');
  succeeded(run(f.root, 'upsert', path, commandHead(f.project.commonDir), input));
  const previous = commandHead(f.project.commonDir);
  expect(git(f.root, ['show', `command:${path}`])).toBe('# Updated\n');
  succeeded(run(f.root, 'delete', path, previous));
  expect(() => git(f.root, ['show', `command:${path}`])).toThrow();
  ...
  expect(readdirSync(f.project.commonDir).filter(p => p.startsWith('command-crud.'))).toEqual([]);
...
test('primer validates edits and broken deletions and refuses stale revisions', () => {
  const f = setup(), input = join(f.project.commonDir, 'prepared.md');
  const path = '.command/tasks/T-001.md', original = commandHead(f.project.commonDir);
  writeFileSync(input, task('T-001', 'planned', 'depends_on: [MISSING]\n'));
  expect(run(f.root, 'upsert', path, original, input).status).not.toBe(0);
  expect(commandHead(f.project.commonDir)).toBe(original);
  ...
  const changed = commandHead(f.project.commonDir);
  expect(snapshot(f.project).tasks[1].ready).toBe(true);
  writeFileSync(input, task('T-001', 'cancelled'));
  expect(run(f.root, 'upsert', path, original, input).status).not.toBe(0);
  expect(commandHead(f.project.commonDir)).toBe(changed);
}, 15_000);
...
test('primer initializes an independent orphan history and refuses reinitialization', () => {
  ...
  writeFileSync(input, `format: repo-project\nformat_version: "0.2"\nid: ${randomUUID()}\nname: Primer project\nplanning_branch: command\ncode_branch: master\n`);
  succeeded(run(root, 'init', '.command/project.yaml', '', input));
  expect(git(root, ['rev-list', '--parents', '--max-count=1', 'command']).trim().split(' ')).toHaveLength(1);
  expect(() => git(root, ['merge-base', 'master', 'command'])).toThrow();
  expect(git(root, ['ls-tree', '--name-only', 'command']).trim()).toBe('.command');
  ...
  const project = f.registry.register(root), head = commandHead(project.commonDir);
  expect(snapshot(project).errors).toEqual([]);
  expect(run(root, 'init', '.command/project.yaml', '', input).status).not.toBe(0);
  expect(commandHead(project.commonDir)).toBe(head);
}, 15_000);

test('primer refuses unsafe paths and a checked-out planning branch', () => {
  const f = setup(), input = join(f.project.commonDir, 'prepared.md'), head = commandHead(f.project.commonDir);
  writeFileSync(input, '# Document\n');
  for (const path of ['source.md', '.command/../source.md', '.command/docs\\bad.md', '.command/docs//bad.md', '.command/.git/config']) {
    expect(run(f.root, 'upsert', path, head, input).status).not.toBe(0);
  }
  git(f.root, ['worktree', 'add', join(f.dir, 'planning'), 'command']);
  expect(run(f.root, 'upsert', '.command/docs/new.md', head, input).status).not.toBe(0);
  expect(commandHead(f.project.commonDir)).toBe(head);
});
