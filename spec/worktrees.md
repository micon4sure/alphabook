# One shared plan, many code worktrees — draft 0.2

The planning branch is `command`. It has independent history and contains only
`.command/`. Code branches contain code. Main/master naming does not affect
planning; no branch is renamed by Command.
...
1. Plan tasks and dependencies on command.
2. Create code worktrees using ordinary Git and the project's existing code base.
3. Update each task on command: status, assignee and code branch names.
...
6. Integrate code using the project's normal workflow. Never merge command into
   a code branch or a code branch into command.
...
Readers use Git objects at an exact command commit. Writers use an isolated
...
in the agent primer work without them. An optional command checkout is supported
for ordinary file editing; direct-ref writers refuse while command is checked
...
Separate clones do not share live state. Fetch and push both command and code
branches explicitly. Reconcile divergent command histories as ordinary Git
